    ...
    const submitForm = async () => {
        ...
        if (user.name && user.email && user.password && user.role
            ...
            if (user.name.length <= 3) {
                ...
                setAlert({ message: 'Enter  Stronge Password', display: true })
                    ...
                    <div role="alert" className="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                        <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                            <div>
                                ...
                                <h2 className="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900 mb-10">Register new account</h2>
                            ...
                            <form>
                                ...
                                <input type={'text'} id="name" className="pmb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="Enter Your Name " value={user.name} onChange={onChange} name='name' />
                            ...
                            <div className="flex items-center justify-start w-full">
                                <button className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 text-sm" onClick={submitForm}>Register</button>
                                <button className="focus:outline-none focus:ring-2 focus:ring-offset-2  focus:ring-gray-400 ml-3 bg-gray-100 transition duration-150 text-gray-600 ease-in-out hover:border-gray-400 hover:bg-gray-300 border rounded px-8 py-2 text-sm" onClick={handleLogin}>Login</button>
                            </div>
